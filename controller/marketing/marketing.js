...
export default class Marketing {
  ...
  getHotel = (req, res) => {
    var sql = "SELECT a.IDXX_HTLX, a.NAMA_HTLX, a.BINTG_HTLX,b.CODD_DESC FROM m_hotel a INNER JOIN tb00_basx b ON a.BINTG_HTLX = b.CODD_VALU WHERE b.CODD_FLNM = 'BINTG_HTLX' ORDER BY a.CRTX_DATE DESC";
    db.query(sql, function (err, rows, fields) {
      res.send(rows);
    })
  }
  ...
  getDetailJadwalJamaah = (req, res) => {
    ...
    var sql = `SELECT a.KDXX_DFTR, a.PEMB_PSPR, a.PRSS_VKSN, a.HANDLING, b.*, TIMESTAMPDIFF(year,b.TGLX_LHIR,CURDATE()) AS UMUR, IFNULL((SELECT SUM(c.TOTL_TGIH) FROM mrkt_tagihanh c WHERE c.KDXX_DFTR = a.KDXX_DFTR),0) AS EST_TOTAL, IFNULL((SELECT SUM(c.JMLX_BYAR) FROM mrkt_tagihanh c WHERE c.KDXX_DFTR = a.KDXX_DFTR),0) AS MASUK, IFNULL((SELECT SUM(c.SISA_TGIH) FROM mrkt_tagihanh c WHERE c.KDXX_DFTR = a.KDXX_DFTR),0) AS SISA, IF(a.STAS_BYAR = 1, 'Lunas', 'Belum') AS STATUS_BAYAR FROM mrkt_daftarh a LEFT JOIN jmah_jamaahh b ON a.KDXX_JMAH = b.NOXX_IDNT WHERE a.KDXX_PKET = '${id}'`
