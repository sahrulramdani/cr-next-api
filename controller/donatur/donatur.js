...
export default class Donatur {
    ...
    getSummaryTransactionPerGroup2 = function(req, res) {
       ...
       /*  var period = '%';
        ...
        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select l.KECX_DESC, l.AREA_DESC, l.AREA_IDXX, k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.AREA_IDXX, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX left join grpx_relx k on g.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX  WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And h.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX";

                    break;
                case '05' :  // bendahara
                    sql = "select k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.KECX_DESC, l.AREA_IDXX, l.AREA_DESC, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet f on a.KodeNik = f.RelawanID left join tb21_empl h on a.KodeNik = h.KodeNik left join grpx_relx k on f.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And f.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by k.NAMA_GRPX";
        ...
        if (typePerson === '4') {  // 4: Official setingkat Relawan Korwil ('32')
            sql = "select Case When h.IDXX_GRPX Is Null Then 'OFISIAL' Else i.KOTA_DESC End As NAMA_GRPX2, i.KOTA_DESC, i.KECX_IDXX, i.KECX_DESC, i.AREA_IDXX, i.AREA_DESC, h.NAMA_GRPX, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And (h.KodeKelurahan like '32%' OR h.IDXX_GRPX is null) And (a.isDelete <> '1' OR a.isDelete IS NULL) group by i.KOTA_DESC, i.KECX_DESC, h.NAMA_GRPX";
        }
    ...
    getSummaryTransactionPerRelawan = function(req, res) {
       ...
       /*  var period = '%';
        ...
        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.AREA_IDXX, m.KodeNik, m.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX left join grpx_relx k on g.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX left join tb21_empl m on a.KodeNik = m.KodeNik  WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And h.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX, m.KodeNik, m.NamaKry";

                    break;
                case '05' :  // bendahara
                    sql = "select k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.KECX_DESC, l.AREA_IDXX, l.AREA_DESC, h.KodeNik, h.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet f on a.KodeNik = f.RelawanID left join tb21_empl h on a.KodeNik = h.KodeNik left join grpx_relx k on f.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And f.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by k.NAMA_GRPX, h.KodeNik, h.NamaKry";
    ...
    getSummaryTransactionPerMonth = function(req, res) {
        ...
        sql = "select CONCAT(GetMidMonth(a.TransDate), ' ', YEAR(a.TransDate)) As BulanTahun, SUM(a.Amount) As JumlahDonasi, SUM(Case a.isValidate When '1' Then a.Amount Else 0 End) As JumlahValidasi, SUM(Case d.CHKX_BANK When '1' Then a.Amount Else 0 End) As JumlahTransfer, SUM(Case d.CHKX_CASH When '1' Then a.Amount Else 0 End) As JumlahTunai, SUM(Case a.isValidate When '1' Then 1 Else 0 End) As JumlahTransValidasi, COUNT(DISTINCT b.NO_ID) As JumlahDonatur FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb02_bank d on a.MethodPayment = d.KODE_BANK And d.KODE_FLNM = 'TYPE_BYRX' And a.BUSS_CODE = d.BUSS_CODE WHERE c.KODE_URUT like '" + req.KODE_URUT0 + "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And (a.isDelete <> '1' OR a.isDelete IS NULL) Group By DATE_FORMAT(a.TransDate, '%Y-%c')";
