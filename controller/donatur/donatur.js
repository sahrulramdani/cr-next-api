...
export default class Donatur {
        ...
        if (status === "0") {   // All Status kecuali New dan Send Back
            qryCmd = "select a.NO_ID as id, a.NAMA, " +
                     "CASE a.JNKX_KLMN " +
                        ...
                        "ELSE 'Perempuan' " +
                      "END As Jns_Kelamin, " + 
                      "a.Email, a.NoHP, b.CODD_DESC As Channel, CONCAT(a.NAMA, ', ', IFNULL(a.TITLE, '')) As Nama2, CONCAT(IFNULL(a.CodeCountryHP, ''), a.NoHP) As NoHP2 from tb11_mzjb a LEFT JOIN tb00_basx b ON a.Channel = b.CODD_VALU And b.CODD_FLNM = 'CHANNEL_DONATUR' INNER JOIN tb00_unit c ON a.BUSS_CODE = c.KODE_UNIT where a.Status <> '1' And a.Status <> '3' And c.KODE_URUT like '" + request.KODE_URUT0 + "%'";
        } else {
            qryCmd = "select a.NO_ID as id, a.NAMA, " +
                     "CASE a.JNKX_KLMN " +
                        "WHEN '1' THEN 'Laki-laki' " +
    ...
    getDonaturTransactions = function(req, res) {
        ...
        if (isValid === 'all') {
            ...
            if (typePerson === '1') {  // 1: Relawan
                ...
                sql = 'SELECT a.*, o.NamaKry As NamaRelawan, b.NAMA, CONCAT(b.NO_ID, " - ", b.NAMA) As Donatur2, DATE_FORMAT(a.TransDate, "%Y%m%d %H:%i:%s") As TransDateFormat, DATE_FORMAT(a.TransDate, "%d/%m/%Y %H:%i:%s") As TglFormat, DATE_FORMAT(a.TransDate, "%Y%m") As yearMonth, d.CODD_DESC As Channel, e.TahunDonasi, b.TITLE, CONCAT(IFNULL(b.CodeCountryHP, ""), b.NoHP) As NoHP2, g.Department, h.NAMA_BANK As Bank, i.CODD_DESC As SegmenProfil, c.NAMA_UNIT, CASE b.Stat_aktf When "1" Then "AKTIF" Else "NON-AKTIF" END As Active, a.Amount, Case j.CHKX_CASH When "1" Then "TUNAI" Else "TRANSFER" End As ChkTunai FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID And e.DonaturID is not null left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join tb02_bank j on a.MethodPayment = j.KODE_BANK And a.BUSS_CODE = j.BUSS_CODE And j.KODE_FLNM = "TYPE_BYRX" left join vfirst_relawandet k on a.KodeNik = k.RelawanID left join tb21_empl o on a.KodeNik = o.KodeNik WHERE ((j.CHKX_CASH = "1" And a.Status in ("' + isValid + ')) Or (j.CHKX_CASH = "0" And a.Status in ("' + isValidTransfer + '")) Or (j.CHKX_CASH = "1" And k.groupID Is Null And a.Status in ("' + isValid4 + '"))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "%' + req.KODE_URUT0 + '%" And IFNULL(g.Department, "") like "' + department + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And IFNULL(o.NamaKry, "") like "%' + relawan + '%" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';
            }
        }
            ...
            if (typePerson === '1') {  // 1: Relawan
                switch(typeRelawan) {
                    case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                        ...
                        sql = 'SELECT a.*, o.NamaKry As NamaRelawan, b.NAMA, CONCAT(b.NO_ID, " - ", b.NAMA) As Donatur2, DATE_FORMAT(a.TransDate, "%Y%m%d %H:%i:%s") As TransDateFormat, DATE_FORMAT(a.TransDate, "%d/%m/%Y %H:%i:%s") As TglFormat, DATE_FORMAT(a.TransDate, "%Y%m") As yearMonth, d.CODD_DESC As Channel, e.TahunDonasi, b.TITLE, CONCAT(IFNULL(b.CodeCountryHP, ""), b.NoHP) As NoHP2, g.Department, h.NAMA_BANK As Bank, i.CODD_DESC As SegmenProfil, c.NAMA_UNIT, CASE b.Stat_aktf When "1" Then "AKTIF" Else "NON-AKTIF" END As Active, a.Amount, Case n.CHKX_CASH When "1" Then "TUNAI" Else "TRANSFER" End As ChkTunai, p.CODD_DESC As Status FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join vfirst_relawandet l on a.KodeNik = l.RelawanID left join grpx_relx m on l.groupID = m.IDXX_GRPX left join tb02_bank n on a.MethodPayment = n.KODE_BANK And a.BUSS_CODE = n.BUSS_CODE And n.KODE_FLNM = "TYPE_BYRX" left join tb21_empl o on a.KodeNik = o.KodeNik left join tb00_basx p on p.CODD_FLNM = "STATUS_TRANSACTION" And a.Status = p.CODD_VALU WHERE ((n.CHKX_CASH = "1" And a.Status in ("' + isValid + ')) Or (n.CHKX_CASH = "0" And a.Status in ("' + isValidTransfer + '"))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "' + req.KODE_URUT0 + '%" And m.KodeKelurahan like "' + req.KODE_AREA0 + '%" And IFNULL(g.Department, "") like "' + department + '%" And o.NamaKry like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';

                        break;
                    case '05' :  // Bendahara
                        sql = 'SELECT a.*, o.NamaKry As NamaRelawan, b.NAMA, CONCAT(b.NO_ID, " - ", b.NAMA) As Donatur2, DATE_FORMAT(a.TransDate, "%Y%m%d %H:%i:%s") As TransDateFormat, DATE_FORMAT(a.TransDate, "%d/%m/%Y %H:%i:%s") As TglFormat, DATE_FORMAT(a.TransDate, "%Y%m") As yearMonth, d.CODD_DESC As Channel, e.TahunDonasi, b.TITLE, CONCAT(IFNULL(b.CodeCountryHP, ""), b.NoHP) As NoHP2, g.Department, h.NAMA_BANK As Bank, i.CODD_DESC As SegmenProfil, c.NAMA_UNIT, CASE b.Stat_aktf When "1" Then "AKTIF" Else "NON-AKTIF" END As Active, a.Amount, Case n.CHKX_CASH When "1" Then "TUNAI" Else "TRANSFER" End As ChkTunai, p.CODD_DESC As Status FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join vfirst_relawandet l on a.KodeNik = l.RelawanID left join grpx_relx m on l.groupID = m.IDXX_GRPX left join tb02_bank n on a.MethodPayment = n.KODE_BANK And a.BUSS_CODE = n.BUSS_CODE And n.KODE_FLNM = "TYPE_BYRX" left join tb21_empl o on a.KodeNik = o.KodeNik left join tb00_basx p on p.CODD_FLNM = "STATUS_TRANSACTION" And a.Status = p.CODD_VALU WHERE ((n.CHKX_CASH = "1" And a.Status in ("' + isValid + ')) Or (n.CHKX_CASH = "0" And a.Status in ("' + isValid + '))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "' + req.KODE_URUT0 + '%" And m.IDXX_GRPX = "' + req.groupID + '" And IFNULL(g.Department, "") like "' + department + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And o.NamaKry like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';

                        break;
                    case '06' :
                        sql = 'SELECT a.*, o.NamaKry As NamaRelawan, b.NAMA, CONCAT(b.NO_ID, " - ", b.NAMA) As Donatur2, DATE_FORMAT(a.TransDate, "%Y%m%d %H:%i:%s") As TransDateFormat, DATE_FORMAT(a.TransDate, "%d/%m/%Y %H:%i:%s") As TglFormat, DATE_FORMAT(a.TransDate, "%Y%m") As yearMonth, d.CODD_DESC As Channel, e.TahunDonasi, b.TITLE, CONCAT(IFNULL(b.CodeCountryHP, ""), b.NoHP) As NoHP2, g.Department, h.NAMA_BANK As Bank, i.CODD_DESC As SegmenProfil, c.NAMA_UNIT, CASE b.Stat_aktf When "1" Then "AKTIF" Else "NON-AKTIF" END As Active, a.Amount, Case n.CHKX_CASH When "1" Then "TUNAI" Else "TRANSFER" End As ChkTunai, p.CODD_DESC As Status FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join tb01_lgxh l on a.KodeNik = l.NO_ID left join tb02_bank n on a.MethodPayment = n.KODE_BANK And a.BUSS_CODE = n.BUSS_CODE And n.KODE_FLNM = "TYPE_BYRX" left join tb21_empl o on a.KodeNik = o.KodeNik left join tb00_basx p on p.CODD_FLNM = "STATUS_TRANSACTION" And a.Status = p.CODD_VALU WHERE ((n.CHKX_CASH = "1" And a.Status in ("' + isValid + ')) Or (n.CHKX_CASH = "0" And a.Status in ("' + isValid + '))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "' + req.KODE_URUT0 + '%" And UPPER(l.USER_IDXX) = "' + req.userID.toUpperCase() + '" And IFNULL(g.Department, "") like "' + department + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And o.NamaKry like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';
                }
            } else if (typePerson === '4') {    // 4: Ofisial
                if (isValid === '1",null') {
                    isValid = '1"';
                ...
                sql = 'SELECT a.*, o.NamaKry As NamaRelawan, b.NAMA, CONCAT(b.NO_ID, " - ", b.NAMA) As Donatur2, DATE_FORMAT(a.TransDate, "%Y%m%d %H:%i:%s") As TransDateFormat, DATE_FORMAT(a.TransDate, "%d/%m/%Y %H:%i:%s") As TglFormat, DATE_FORMAT(a.TransDate, "%Y%m") As yearMonth, d.CODD_DESC As Channel, e.TahunDonasi, b.TITLE, CONCAT(IFNULL(b.CodeCountryHP, ""), b.NoHP) As NoHP2, g.Department, h.NAMA_BANK As Bank, i.CODD_DESC As SegmenProfil, c.NAMA_UNIT, CASE b.Stat_aktf When "1" Then "AKTIF" Else "NON-AKTIF" END As Active, a.Amount, Case j.CHKX_CASH When "1" Then "TUNAI" Else "TRANSFER" End As ChkTunai, p.CODD_DESC As Status FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID And e.DonaturID is not null left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join tb02_bank j on a.MethodPayment = j.KODE_BANK And a.BUSS_CODE = j.BUSS_CODE And j.KODE_FLNM = "TYPE_BYRX" left join vfirst_relawandet k on a.KodeNik = k.RelawanID left join tb21_empl o on a.KodeNik = o.KodeNik left join tb00_basx p on p.CODD_FLNM = "STATUS_TRANSACTION" And a.Status = p.CODD_VALU WHERE ((j.CHKX_CASH = "1" And a.Status in ("' + isValid + ')) Or (j.CHKX_CASH = "0" And a.Status in ("' + isValidTransfer + '")) Or (j.CHKX_CASH = "1" And k.groupID Is Null And a.Status in ("' + isValid4 + '"))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "%' + req.KODE_URUT0 + '%" And IFNULL(g.Department, "") like "' + department + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And IFNULL(o.NamaKry, "") like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';
            }
        }
            ...
            if (typePerson === '1') {  // 1: Relawan
                switch(typeRelawan) {
                    case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                        ...
                        break;
                    case '05' :  // Bendahara
                        sql = 'SELECT COUNT(*) As TotalData FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join vfirst_relawandet l on a.KodeNik = l.RelawanID left join grpx_relx m on l.groupID = m.IDXX_GRPX left join tb02_bank n on a.MethodPayment = n.KODE_BANK And a.BUSS_CODE = n.BUSS_CODE And n.KODE_FLNM = "TYPE_BYRX" left join tb21_empl o on a.KodeNik = o.KodeNik WHERE ((n.CHKX_CASH = "1" And a.isValidate in ("' + isValid + ')) Or (n.CHKX_CASH = "0" And a.isValidate in ("' + isValid + '))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "' + req.KODE_URUT0 + '%" And m.IDXX_GRPX = "' + req.groupID + '" And IFNULL(g.Department, "") like "' + department + '%" And o.NamaKry like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';
                ...
                sql = 'SELECT COUNT(*) As TotalData FROM trans_donatur a left join tb11_mzjb b on a.donaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx d on b.Channel = d.CODD_VALU And d.CODD_FLNM = "CHANNEL_DONATUR" left join vfirst_transaction e on a.DonaturID = e.DonaturID And e.DonaturID is not null left join vdepartment g on a.CRTX_BYXX = g.USER_IDXX left join tb02_bank h on a.BankTo = h.KODE_BANK And h.KODE_FLNM = "KASX_BANK" left join tb00_basx i on b.SEGMX_PROF = i.CODD_VALU And i.CODD_FLNM = "SEGMENT_PROFILING" left join tb02_bank j on a.MethodPayment = j.KODE_BANK And a.BUSS_CODE = j.BUSS_CODE And j.KODE_FLNM = "TYPE_BYRX" left join vfirst_relawandet k on a.KodeNik = k.RelawanID left join tb21_empl o on a.KodeNik = o.KodeNik WHERE ((j.CHKX_CASH = "1" And a.isValidate in ("' + isValid + ')) Or (j.CHKX_CASH = "0" And a.isValidate = "' + isValidTransfer + '") Or (j.CHKX_CASH = "1" And k.groupID Is Null And a.isValidate in ("' + isValid4 + '"))) And (a.isDelete <> "1" OR a.isDelete IS NULL) And c.KODE_URUT like "%' + req.KODE_URUT0 + '%" And IFNULL(g.Department, "") like "' + department + '%" And DATE_FORMAT(a.TransDate, "%Y%m") like "' + yearMonth + '%" And o.NamaKry like "%' + relawan + '%" And DATE_FORMAT(a.TransDate, "%Y-%m-%d") between "' + fromDate + '" And "' + toDate + '" order by c.KODE_URUT, a.TransDate Desc, a.TransNumber';
            }
        }
            ...
            if (err) {
                ...
                db.query(sql, (err, result) => {
                    ...
                    db.query(sql, (err, result) => {
                        ...
                        if (req.body.Status === '2') {   // 2: Status is Validated
                            ...
                            db.query(sql, function(err, rows, fields) {
                                if (rows !== undefined && rows[0].length > 0) {   // if validate
                                    ...
                                    if (req.body.Status === '3') {  // 3: Not Found
                                        sql = 'select a.*, CONCAT(b.CodeCountryHP, b.NoHP) As NoHP from trans_donatur a inner join tb11_mzjb b on a.DonaturID = b.NO_ID where a.id = ' + id;
                                        db.query(sql, function(err, rows, fields) {
                                            // kirim WA Chatbot
                                            var data2 = {
    ...
    getSummaryTransactionPerProgram = function(req, res) {
        ...
        var sql = "select IFNULL(d.CODD_DESC, '') As ProgramDonatur, IFNULL(e.CODD_DESC, '') As SegmenProfil, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(f.Amount_item) As JumlahTransaksi, SUM(f.Amount_item) As JumlahDonasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join trans_item f on a.TransNumber = f.TransNumber left join tb00_basx d on f.ProgDonatur = d.CODD_VALU And f.BUSS_CODE = d.CODD_VARC And d.CODD_FLNM = 'PROGRAM_DONATUR' WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And f.ProgDonatur is not null And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' group by d.CODD_DESC";

        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select IFNULL(d.CODD_DESC, '') As ProgramDonatur, IFNULL(e.CODD_DESC, '') As SegmenProfil, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(f.Amount_item) As JumlahTransaksi, SUM(f.Amount_item) As JumlahDonasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join trans_item f on a.TransNumber = f.TransNumber left join tb00_basx d on f.ProgDonatur = d.CODD_VALU And f.BUSS_CODE = d.CODD_VARC And d.CODD_FLNM = 'PROGRAM_DONATUR' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And f.ProgDonatur is not null And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And h.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' group by d.CODD_DESC";

                    break;
                case '05' :  // Bendahara
                    sql = "select IFNULL(d.CODD_DESC, '') As ProgramDonatur, IFNULL(e.CODD_DESC, '') As SegmenProfil, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(f.Amount_item) As JumlahTransaksi, SUM(f.Amount_item) As JumlahDonasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join trans_item f on a.TransNumber = f.TransNumber left join tb00_basx d on f.ProgDonatur = d.CODD_VALU And f.BUSS_CODE = d.CODD_VARC And d.CODD_FLNM = 'PROGRAM_DONATUR' left join vfirst_relawandet g on a.KodeNik = g.RelawanID WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And f.ProgDonatur is not null And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And g.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' group by d.CODD_DESC";

                    break;
                case '06' : 
                    sql = "select IFNULL(d.CODD_DESC, '') As ProgramDonatur, IFNULL(e.CODD_DESC, '') As SegmenProfil, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(f.Amount_item) As JumlahTransaksi, SUM(f.Amount_item) As JumlahDonasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join trans_item f on a.TransNumber = f.TransNumber left join tb00_basx d on f.ProgDonatur = d.CODD_VALU And f.BUSS_CODE = d.CODD_VARC And d.CODD_FLNM = 'PROGRAM_DONATUR' inner join tb01_lgxh g on a.KodeNik = g.NO_ID WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And f.ProgDonatur is not null And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And UPPER(g.USER_IDXX) = '" + req.userID.toUpperCase() + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' group by d.CODD_DESC";
            }
        }
        ...
        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    ...
                    break;
                case '05' :  // Bendahara
                    sql = "select DATE_FORMAT(a.TransDate,'%Y-%m') As TahunBulan, CONCAT(MONTHNAME(a.TransDate),' ',YEAR(a.TransDate)) As BulanTahun, d.CODD_DESC As ProgramDonatur, IFNULL(e.CODD_DESC, '') As SegmenProfil, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(f.Amount_item) As JumlahTransaksi, SUM(f.Amount_item) As JumlahDonasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join trans_item f on a.TransNumber = f.TransNumber left join tb00_basx d on f.ProgDonatur = d.CODD_VALU And f.BUSS_CODE = d.CODD_VARC And d.CODD_FLNM = 'PROGRAM_DONATUR' WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And f.ProgDonatur is not null And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And a.KODE_KLSX = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' group by DATE_FORMAT(a.TransDate,'%Y-%m') Desc, d.CODD_DESC, e.CODD_DESC";
    ...
    getSummaryTransactionPerGroup2 = function(req, res) {
       ...
       /*  var period = '%';
        ...
        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select l.KECX_DESC, l.AREA_DESC, l.AREA_IDXX, k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.AREA_IDXX, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX left join grpx_relx k on g.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX  WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And h.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX";

                    break;
                case '05' :  // bendahara
                    sql = "select k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.KECX_DESC, l.AREA_IDXX, l.AREA_DESC, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet f on a.KodeNik = f.RelawanID left join tb21_empl h on a.KodeNik = h.KodeNik left join grpx_relx k on f.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And f.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by k.NAMA_GRPX";
    ...
    getSummaryTransactionPerRelawan = function(req, res) {
       ...
       /*  var period = '%';
        ...
        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.AREA_IDXX, m.KodeNik, m.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX left join grpx_relx k on g.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX left join tb21_empl m on a.KodeNik = m.KodeNik  WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And h.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by l.KECX_DESC, l.AREA_DESC, k.NAMA_GRPX, m.KodeNik, m.NamaKry";

                    break;
                case '05' :  // bendahara
                    sql = "select k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.KECX_DESC, l.AREA_IDXX, l.AREA_DESC, h.KodeNik, h.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet f on a.KodeNik = f.RelawanID left join tb21_empl h on a.KodeNik = h.KodeNik left join grpx_relx k on f.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And f.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by k.NAMA_GRPX, h.KodeNik, h.NamaKry";

                    break;
                case '06' :
                    sql = "select k.NAMA_GRPX, l.KOTA_DESC, l.KECX_IDXX, l.KECX_DESC, l.AREA_IDXX, l.AREA_DESC, j.KodeNik, j.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' inner join tb01_lgxh g on a.KodeNik = g.NO_ID left join vfirst_relawandet h on a.KodeNik = h.RelawanID left join tb21_empl j on a.KodeNik = j.KodeNik left join grpx_relx k on h.groupID = k.IDXX_GRPX left join tb20_area l on k.KodeKelurahan = l.AREA_IDXX WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And UPPER(g.USER_IDXX) = '" + req.userID.toUpperCase() + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) group by k.NAMA_GRPX, j.KodeNik, j.NamaKry";
            }
        }

        if (typePerson === '4') {  // 4: Official setingkat Relawan Korwil ('32')
            sql = "select Case When h.IDXX_GRPX Is Null Then 'OFISIAL' Else i.KOTA_DESC End As NAMA_GRPX2, i.KOTA_DESC, i.KECX_IDXX, i.KECX_DESC, i.AREA_IDXX, i.AREA_DESC, h.NAMA_GRPX, j.KodeNik, j.NamaKry, COUNT(distinct a.DonaturID) As JumlahDonatur, COUNT(a.Amount) As JumlahTransaksi, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahTerverifikasi FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb00_basx e on b.SEGMX_PROF = e.CODD_VALU And e.CODD_FLNM = 'SEGMENT_PROFILING' left join vfirst_relawandet g on a.KodeNik = g.RelawanID left join grpx_relx h on g.groupID = h.IDXX_GRPX left join tb20_area i on h.KodeKelurahan = i.AREA_IDXX left join tb21_empl j on a.KodeNik = j.KodeNik WHERE c.KODE_URUT like '" + req.KODE_URUT0 +  "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And (h.KodeKelurahan like '32%' OR h.IDXX_GRPX is null) And (a.isDelete <> '1' OR a.isDelete IS NULL) group by i.KOTA_DESC, i.KECX_DESC, h.NAMA_GRPX, j.KodeNik, j.NamaKry";
        }
    ...
    getSummaryTransactionPerMonth = function(req, res) {
        ...
        sql = "select CONCAT(GetMidMonth(a.TransDate), ' ', YEAR(a.TransDate)) As BulanTahun, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahValidasi, SUM(Case d.CHKX_BANK When '1' Then a.Amount Else 0 End) As JumlahTransfer, SUM(Case d.CHKX_CASH When '1' Then a.Amount Else 0 End) As JumlahTunai, SUM(Case a.Status When '2' Then 1 Else 0 End) As JumlahTransValidasi, COUNT(DISTINCT b.NO_ID) As JumlahDonatur FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb02_bank d on a.MethodPayment = d.KODE_BANK And d.KODE_FLNM = 'TYPE_BYRX' And a.BUSS_CODE = d.BUSS_CODE WHERE c.KODE_URUT like '" + req.KODE_URUT0 + "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 +   "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' Group By DATE_FORMAT(a.TransDate, '%Y-%c')";

        if (typePerson === '1') {
            switch(typeRelawan) {
                case '01' : case '02' : case '03' : case '04' :   // 04: Korra
                    sql = "select CONCAT(GetMidMonth(a.TransDate), ' ', YEAR(a.TransDate)) As BulanTahun, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahValidasi, SUM(Case d.CHKX_BANK When '1' Then a.Amount Else 0 End) As JumlahTransfer, SUM(Case d.CHKX_CASH When '1' Then a.Amount Else 0 End) As JumlahTunai, SUM(Case a.Status When '2' Then 1 Else 0 End) As JumlahTransValidasi, COUNT(DISTINCT b.NO_ID) As JumlahDonatur FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb02_bank d on a.MethodPayment = d.KODE_BANK And d.KODE_FLNM = 'TYPE_BYRX' And a.BUSS_CODE = d.BUSS_CODE left join vfirst_relawandet e on a.KodeNik = e.RelawanID left join grpx_relx f on e.groupID = f.IDXX_GRPX WHERE c.KODE_URUT like '" + req.KODE_URUT0 + "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 + "' And f.KodeKelurahan like '" + req.KODE_AREA0 + "%' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' Group By DATE_FORMAT(a.TransDate, '%Y-%c')";

                    break;
                case '05' :  // Bendahara
                    sql = "select CONCAT(GetMidMonth(a.TransDate), ' ', YEAR(a.TransDate)) As BulanTahun, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahValidasi, SUM(Case d.CHKX_BANK When '1' Then a.Amount Else 0 End) As JumlahTransfer, SUM(Case d.CHKX_CASH When '1' Then a.Amount Else 0 End) As JumlahTunai, SUM(Case a.Status When '2' Then 1 Else 0 End) As JumlahTransValidasi, COUNT(DISTINCT b.NO_ID) As JumlahDonatur FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb02_bank d on a.MethodPayment = d.KODE_BANK And d.KODE_FLNM = 'TYPE_BYRX' And a.BUSS_CODE = d.BUSS_CODE left join vfirst_relawandet e on a.KodeNik = e.RelawanID WHERE c.KODE_URUT like '" + req.KODE_URUT0 + "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 + "' And e.groupID = '" + req.groupID + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) Group By DATE_FORMAT(a.TransDate, '%Y-%c') And a.Status <> '3' Group By DATE_FORMAT(a.TransDate, '%Y-%c')";

                    break;
                case '06' :  // 06: Relawan
                    sql = "select CONCAT(GetMidMonth(a.TransDate), ' ', YEAR(a.TransDate)) As BulanTahun, SUM(a.Amount) As JumlahDonasi, SUM(Case a.Status When '2' Then a.Amount Else 0 End) As JumlahValidasi, SUM(Case d.CHKX_BANK When '1' Then a.Amount Else 0 End) As JumlahTransfer, SUM(Case d.CHKX_CASH When '1' Then a.Amount Else 0 End) As JumlahTunai, SUM(Case a.Status When '2' Then 1 Else 0 End) As JumlahTransValidasi, COUNT(DISTINCT b.NO_ID) As JumlahDonatur FROM trans_donatur a left join tb11_mzjb b on a.DonaturID = b.NO_ID inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT left join tb02_bank d on a.MethodPayment = d.KODE_BANK And d.KODE_FLNM = 'TYPE_BYRX' And a.BUSS_CODE = d.BUSS_CODE inner join tb01_lgxh e on a.KodeNik = e.NO_ID WHERE c.KODE_URUT like '" + req.KODE_URUT0 + "%' And DATE_FORMAT(a.TransDate, '%Y%m%d') Between '" + tgl1 + "' And '" + tgl2 + "' And UPPER(e.USER_IDXX) = '" + req.userID.toUpperCase() + "' And (a.isDelete <> '1' OR a.isDelete IS NULL) And a.Status <> '3' Group By DATE_FORMAT(a.TransDate, '%Y-%c')";
            }
        }
    ...
    saveTransItemArray = function(req, res) {
        ...
        if (cntTransItems > 0) {
            ...
            sql = 'INSERT INTO trans_item (TransNumber, BUSS_CODE, THNX_BUKU, KodeNik, KODE_KLSX, ProgDonatur, Amount_item, note, CRTX_DATE, CRTX_BYXX, UPDT_DATE) VALUES ';
            for(var i=0; i<cntTransItems; i++) {
                if (i === 0) {
                    sql += '("' + req.body.TransNumber + '","' + req.body.BUSS_CODE + '","' + req.body.THNX_BUKU + '","' + req.body.KodeNik + '","' + kelas + '","' + progDonaturs[i] + '",' + amountItems[i] + ',"' + notes[i] + '","' + tglNow + '","' + req.userID + '", "' + tglNow + '")';
                } else {
                    sql += ',("' + req.body.TransNumber + '","' + req.body.BUSS_CODE + '","' + req.body.THNX_BUKU + '","' + req.body.KodeNik + '","' + kelas + '","' + progDonaturs[i] + '",' + amountItems[i] + ',"' + notes[i] + '","' + tglNow + '","' + req.userID + '", "' + tglNow + '")';
                }
            }
            ...
            db.query(sql, (err, result) => {
                if (result.length > 0) {
                    ...
                    const fncNext2 = (initial, tahun2, generateNumber, nextSequenceFormat) => {
                        ...
                        sql = 'INSERT INTO trans_donatur (TransNumber, TransDate, BUSS_CODE, DonaturID, CurrencyID, Amount, MethodPayment, FileName, ProgDonatur, KodeNik, NoInvoice, Status, isDelete, TahunBuku, isSend, CRTX_DATE, CRTX_BYXX, terminal, BankTo, BankFrom) VALUES ("' + generateNumber + '", "' + request.body.TglTransaksi + '", "' + bussCode + '", "' + donaturID + '", "IDR", ' + request.body.Amount + ', "' + request.body.MethodPayment + '", "", "' + request.body.Program + '", (select KodeNik from tb21_empl where Hp = "' + request.body.NoHPRelawan + '" Limit 1), "' + request.body.NoInvoice + '", "' + isValidate + '", "0", (select THNX_AJAR from tb00_thna where CABX_CODE = "' + bussCode + '" And ("' + tglNow2 + '" Between DATE_FORMAT(TGLX_STRT, "%Y-%m-%d") And DATE_FORMAT(TGLX_ENDX, "%Y-%m-%d") Or STAT_AKTF = "1") Limit 1), "' + isSend + '", "' + tglNow + '", "' + request.userID + '", "2" /* 2: WA Chatbot */, (select KODE_KASX_BANK from tb02_bank where KODE_BANK = "' + request.body.MethodPayment + '" And KODE_FLNM = "TYPE_BYRX" And BUSS_CODE = "' + bussCode + '"), "' + request.body.BankFrom + '")';  
                    ...
                    const fncNext = (initial, tahun2, generateNumber, nextSequenceFormat) => {
                        ...
                        db.query(sql, (err, result) => {
                            const fncNext2 = (initial, tahun2, generateNumber, nextSequenceFormat) => {
                                ...
                                // save transaction induk
                                sql = 'INSERT INTO trans_donatur (TransNumber, TransDate, BUSS_CODE, DonaturID, CurrencyID, Amount, FileName, ProgDonatur, KodeNik, NoInvoice, Status, isDelete, TahunBuku, isSend, CRTX_DATE, CRTX_BYXX, MethodPayment, BankTo, terminal, BankFrom) VALUES ("' + generateNumber + '", "' + request.body.TglTransaksi + '", "' + bussCode + '", "' + donaturID + '", "IDR", ' + request.body.Amount + ', "", "' + request.body.Program + '", (select KodeNik from tb21_empl where Hp = "' + request.body.NoHPRelawan + '" Limit 1), "' + request.body.NoInvoice + '", "' + isValidate + '", "0", (select THNX_AJAR from tb00_thna where CABX_CODE = "' + bussCode + '" And ("' + tglNow2 + '" Between DATE_FORMAT(TGLX_STRT, "%Y-%m-%d") And DATE_FORMAT(TGLX_ENDX, "%Y-%m-%d") Or STAT_AKTF = "1") Limit 1), "' + isSend + '", "' + tglNow + '", "' + request.userID + '", "' + request.body.MethodPayment + '", (select KODE_KASX_BANK from tb02_bank where KODE_BANK = "' + request.body.MethodPayment + '" And KODE_FLNM = "TYPE_BYRX" And BUSS_CODE = "' + bussCode + '"), "2", "' + request.body.BankFrom + '")';  
    ...
    verifyTrans = function(req, res) {
        ...
        var sql = 'UPDATE trans_donatur a inner join tb02_bank b on a.MethodPayment = b.KODE_BANK And a.BUSS_CODE = b.BUSS_CODE inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT SET a.Status = "2" /* 2: Validated */, a.UPDT_DATE = "' + tgl + '", a.UPDT_BYXX = "' + req.userID + '", a.isSend = Case a.Status When "2" Then "1" Else "0" End WHERE c.KODE_URUT like "' + req.KODE_URUT0 + '%" And a.Status = "1" And b.CHKX_CASH = "1" And a.id in ("';
    ...
    getTransCash = function(req, res) {
        ...
        var sql = 'select a.*, c.NAMA_UNIT from trans_donatur a inner join tb02_bank b on a.MethodPayment = b.KODE_BANK And a.BUSS_CODE = b.BUSS_CODE inner join tb00_unit c on a.BUSS_CODE = c.KODE_UNIT where a.Status = "1" /* 1: NEW (Not Valdated) */ And c.KODE_URUT like "' + req.KODE_URUT0 + '%" And b.CHKX_CASH = "1" And a.id in ("';
